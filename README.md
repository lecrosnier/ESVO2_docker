...
**Status (2026-09-23):** the pipeline runs in real time and tracking holds. On a 1 m out-and-back slide replayed at 1x, the rig now recovers +1.02 / -0.96 m per leg with no tracking resets, against 41-45% of the true translation a few days earlier; on MVSEC this build matches the paper's published trajectories on all three `indoor_flying` sequences tried (ATE 7.9 / 10.7 / 7.0 cm against the paper's 7.6 / 10.0 / 7.3 cm), once the datasets' 30 ms event batches are repacked. On VECtor `desk-normal` (Prophesee 640x480), vision-only lands in the paper's range at 0.5x replay (ATE 20-22 cm against 16.5 cm) without tuning, and at 1x matches that (21-23 cm) with the depth regularizer off -- it was 93% of the mapping cycle there and buys nothing measurable on this data. Upstream's `USE_IMU: True` mode segfaults on VECtor as it does on MVSEC. What got it there — data rates, stale maps from subscriber-queue backlog, and full-frame work nobody consumed — is written up in [docs/superpowers/specs/2026-09-23-realtime-pipeline-findings.md](docs/superpowers/specs/2026-09-23-realtime-pipeline-findings.md), together with the hypotheses that turned out to be wrong.

The rig was recalibrated on 2026-09-23 after its camera mounts were swapped, which had shifted the cameras by ~1° and lengthened the baseline 5%; in a hallway with tape-measured walls, forward legs now come out within 1-10% (`calib/evk4_stereo_2026-09-23/`, findings doc part B2). Still open: the tracker cannot yet hold its state while the rig stands still, and drift along the optical axis (0.3-0.8 m of closure error on a 2.3 m round trip, against a few cm sideways), which is worst against the flat wall these bags were recorded on. ESVO2's own IMU mode diverged on both public datasets because of an unbounded constant-velocity term in the tracker's translation prior; that is fixed (`IMU_CONSTANT_VELOCITY_PRIOR`, default off), and `USE_IMU: True` now tracks MVSEC and VECtor as well as vision-only, with better path recovery on VECtor. The rig keeps `USE_IMU: False` until it is checked there: its IMU-to-camera lever arm is uncalibrated. The IMU-to-camera rotation is calibrated; the lever arm is not.
